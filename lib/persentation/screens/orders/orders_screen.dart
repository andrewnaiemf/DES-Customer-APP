  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      Future.delayed(const Duration(milliseconds: 500), () {
        if (mounted) {
          final orders = OrdersCubit.get(context).allOrders;
            ...
            children: [
              ...
              BlocBuilder<OrdersCubit, OrdersState>(
                buildWhen: (previous, current) {
                  return current is OrdersGetSuccess ||
                      current is OrdersGetError;
                },
                builder: (context, state) {
                  final cubit = OrdersCubit.get(context);
                  return _StatusFilterWidget(
                    selectedStatus: cubit.activeFilter,
                    isDark: _isDark,
                    onStatusChanged: (status) {
                      HapticFeedback.lightImpact();
                      cubit.filterByStatus(status);
                      
                      // ✅ مزامنة بعد التصفية - طلبات نشطة فقط
                      WidgetsBinding.instance.addPostFrameCallback((_) {
                        if (mounted) {
              ...
              Expanded(
                child: BlocBuilder<OrdersCubit, OrdersState>(
                  builder: (context, state) {
                    ...
                    // ✅ First Load Loading
                    if (state is OrdersGetLoading && state.isFirstLoad) {
                      return const _LoadingState();
                    }

                    // ✅ Error with Cache
                    if (state is OrdersGetError && state.hasCache) {
                      return Column(
                        children: [
                    ...
                    if (state is OrdersGetError && !state.hasCache) {
                      return _EmptyState(
                        ...
                        onRetry: () => cubit.getOrders(forceRefresh: true),
                      );
                    }
  ...
  Widget _buildSearchBar() {
    return Padding(
      ...
      child: TextField(
        ...
        onChanged: (value) => OrdersCubit.get(context).search(value),
        style: TextStyle(
          ...
          suffixIcon: _searchController.text.isNotEmpty
              ? IconButton(
                  ...
                  onPressed: () {
                    ...
                    OrdersCubit.get(context).search('');
                  },
                )
              : null,
          filled: true,
          ...
          onTap: () {
            ...
            if (trackingCubit.state.currentOrderId != null) {
              ...
              MyNavigator.navigateTo(
                context,
                OrderDetailsScreen(orderModel: currentOrder),
              );
            }
          },
        ),
        ...
        Expanded(
          child: RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemCount: orders.length + (cubit.hasMore ? 1 : 0),
                  ...
                  child: Stack(
                    children: [
                      ...
                      Positioned(
                        top: 16,
                        left: 16,
                        child: OrderTrackingIndicator(orderId: orderId),
                      ),
                    ],
                  ),
                );
...
class _StatusFilterWidget extends StatelessWidget {
  ...
  List<_StatusOption> _getAvailableStatuses(BuildContext context) {
    ...
    final allOrders = OrdersCubit.get(context).unfilteredOrders;
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(
              isError ? Icons.error_outline_rounded : Icons.shopping_bag_outlined,
              size: 80,
              color: isDark 
          ...
          Text(
            isError ? 'Error Loading Orders'.tr() : 'No Orders Yet'.tr(),
            style: TextStyle(
              fontSize: 24,
...
class CardOrderWidget extends StatelessWidget {
  ...
  Color get _statusColor {
    if (orderModel.status == "Declined" || orderModel.status == "Canceled") {
                ...
                child: Column(
                  ...
                  children: [
                    _CardHeader(
                      ...
                      is_offer: orderModel.is_offer??false,
...
class _InfoGrid extends StatelessWidget {
  ...
  Widget _getOrderValueWidget(BuildContext context) {
    String amount;
    if (orderModel.loyaltyPoints != null && orderModel.loyaltyPoints != 0) {
      final total = double.parse("${orderModel.total}");
      final discount = double.parse("${orderModel.loyaltyDiscount}");
      final afterDiscount = total - discount;
      final withTax = afterDiscount + (afterDiscount * 0.15);
      amount = "$withTax";
    } else {
      amount = '${orderModel.totalWithTax}';
    }
          ...
          children: [
            Expanded(
              child: _InfoItem(
                ...
                value: formatDate(context, orderModel.checkoutDate!),
                isDark: isDark,
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: _InfoItem(
                ...
                value: formatTime(orderModel.checkoutDate),
                isDark: isDark,
              ),
            ),
          ],
          ...
          Row(
            children: [
              Icon(
                icon,
                size: 14,
                color: isDark ? AppTheme.darkGray : Colors.grey[500],
              ),
              const SizedBox(width: 6),
              Flexible(
                child: Text(
...
class _CardFooter extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final canEdit = orderModel.shippingStatus != OrderShippingStatus.delivered &&
        orderModel.status != OrderStatus.canceled &&
        orderModel.status != OrderStatus.declined;

    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        if (canEdit&&orderModel.is_offer!=true)
          GestureDetector(
            onTap: () {
              HapticFeedback.lightImpact();
              MyNavigator.navigateTo(
                context,
                OrderDetailsScreen(orderModel: orderModel, withEdit: true),
              );
            },
            child: Container(
